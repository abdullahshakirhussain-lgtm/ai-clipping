...
Call submit_metadata with optimized fields.`,
  ...
  async writeStory(input: WriteStoryInput): Promise<StoryScript> {
    ...
    const architectStep1 = isScenario
      ? `STEP 1 — FRAME ONE SCENARIO AS ONE MAN'S DAY, AND HOLD IT. "${input.topic}" is a window into how life actually was — but "generic" is the enemy. Anchor the whole video to ONE SPECIFIC MAN OR BOY the viewer BECOMES (never a woman or girl). Pick a real or realistic man from the period to keep the detail accurate — BUT the viewer never hears his name: do NOT open with or state "you are [Name], a [job]", and do NOT announce a name at all. The viewer is simply "you", a man, living this one actual day in second person, start to finish — a PEACEFUL, contented GOOD day. List 3 such men (as one-line vantage ideas) in "angleOptions", then commit to ONE. THREE HARD RULES, because the usual failure is a script that sprawls:
   - ONE POINT OF VIEW, WHOLE VIDEO. The entire thing is lived from that one vantage in SECOND PERSON ("you"). NEVER cut away to "a real person did this" / "X testified that…" — that shatters the immersion. Concrete detail is WOVEN INTO the walk ("your neighbour the miller always waves you over for the first loaf") — you never leave the vantage point to cite anyone from outside, and you never name yourself.
