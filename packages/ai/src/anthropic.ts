...
1. "stillPrompt" — the SINGLE warm, lived-in still (this becomes the first frame). Describe ONE cosy, richly detailed scene of a peaceful community: the place and its concrete life — homes, gardens, tended fields, animals, food, water, warm hearth/lantern light, the small everyday props that make it feel real — plus the time of day and season. PEOPLE are present and CONTENT (tending a garden, carrying firewood, sharing a meal, fishing, walking home), but shown SMALL, at a distance, or from BEHIND — NEVER a close-up face or portrait. Show NO modern technology (no cars, phones, screens, power lines, plastic). FRAMING: DEFAULT to a high AERIAL / DRONE view looking DOWN on the WHOLE community from the sky — the entire settlement packed into one bird's-eye frame (rooftops, gardens, fields, paths, water, tiny people below). ONLY drop to a closer, lower view when the scene is really about one intimate element (a stream, vegetation, a garden, a hearth); then frame THAT element up close. State the vantage explicitly at the start ("aerial drone view from high above…" or "low close-up view of…"). Positive, concrete description only; do NOT write art-style words (medium/palette are added automatically), and NO on-screen text. Calm and still — a beautiful, inviting held moment you'd want to step into.

2. "motionPrompt" — ONE single, continuous, GENTLE motion for that exact frame, and nothing else: e.g. drifting petals or snow, rolling mist, rising embers/sparks, slow water ripples, tall grass swaying in a soft breeze, a slow camera push-in, a cloak stirring. For an AERIAL/DRONE still, the motion is a slow, smooth aerial drift or a very slow descent over the whole scene; for a CLOSE-UP still, it's gentle motion of the element itself (water flowing, grass and leaves swaying, smoke rising). It must be subtle and loop-friendly — NO cuts, no fast action, no new elements entering, no camera shake. One quiet motion over the still.
