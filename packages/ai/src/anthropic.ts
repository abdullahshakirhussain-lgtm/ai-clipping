...
Call submit_metadata with optimized fields.`,
  ...
  async writeStory(input: WriteStoryInput): Promise<StoryScript> {
    ...
    const beatLengthGuidance = isLong
      ...
      : `BEAT LENGTH IS CRITICAL — every beat is ONE short, simple sentence of about 8-14 words (~3 seconds spoken). NEVER write a long or multi-clause sentence: if a thought has two actions ("you wake before dawn and light the fire"), split it into two beats. This matters because each short beat becomes ONE on-screen picture held ~3 seconds — a long sentence can only ever be shown as 3 pictures, so a 20-word beat sits on screen 6+ seconds and the video stalls. So the BEAT COUNT is a HARD REQUIREMENT: write EXACTLY ${minBeats}-${maxBeats} short beats, no fewer. This is a SHORT, so the whole narration MUST also fit ${minWords}-${maxWords} words (≈2.5 min, under YouTube's 3-minute cap). Reconcile the two like this: choose only the 3-5 most striking moments of the day (do NOT narrate every hour), but tell EACH chosen moment across SEVERAL short beats — not one fat sentence — until you reach ${minBeats}-${maxBeats} beats total. Keep the WAKING open and the FALLING-ASLEEP close. CRUCIAL — short must NOT mean choppy: the beats are read back-to-back as ONE continuous narration, so they must CHAIN smoothly. Each short sentence flows naturally into the next (cause and effect, natural connectors like "then", "and by now", "outside,"), so read end to end it sounds like one warm, flowing piece of storytelling — NEVER a staccato list of disconnected facts ("You wake. It is cold. You stand up."). You are simply telling the same connected story in shorter sentences, not fragmenting it.`;
