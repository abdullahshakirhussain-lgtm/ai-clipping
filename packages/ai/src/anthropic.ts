...
Call submit_metadata with optimized fields.`,
  ...
  async writeStory(input: WriteStoryInput): Promise<StoryScript> {
    ...
    const architectStep1 = isScenario
      ? `STEP 1 — FRAME ONE SCENARIO AS A REAL PERSON, AND HOLD IT. "${input.topic}" is a window into how life actually was — but "generic" is the enemy. Anchor the whole video to ONE REAL, SPECIFIC, NAMED PERSON the viewer BECOMES. THE PROTAGONIST IS ALWAYS A MAN OR BOY — "you" are a man ("you are Aldric, a village baker"; "you are a young farmhand named Tom") — never a woman or girl. Where the history hands you a documented man, use him by name (you ARE a named baker, farmer or fisherman from a real account). Where no single record fits, build ONE representative man out of real documented detail — a real name, a real place, a real year — and commit to him; never a faceless "a person" or "people back then". The viewer lives THIS one man's actual day in second person ("you"), start to finish, and it is a PEACEFUL, contented GOOD day. List 3 such men you weighed in "angleOptions", then commit to ONE. THREE HARD RULES, because the usual failure is a script that sprawls:
   - ONE POINT OF VIEW, WHOLE VIDEO. The entire thing is lived from that one vantage in SECOND PERSON ("you"). NEVER cut away to "a real person did this" / "X testified that…" — that shatters the immersion and turns it into a museum plaque. Real names, dates and detail are still used, but WOVEN INTO the walk ("a neighbour, old Wat the miller, always waves you over for the first loaf") — you never leave the vantage point to cite them from outside.
   - ONE DAY, NOT A SURVEY. Follow a single representative DAY from waking to sleeping, and make it a GOOD, calm day (e.g. the pre-dawn wake in a warm bed → lighting the fire → the walk through the waking village → the day's honest work → supper by the fire with family → drifting off to sleep content). The day is the spine; walk it in order. Do NOT try to cover the whole topic — resist the pull to list every related person, place and event. Better to go deep on one peaceful lived day than wide across ten.
...
The payoff is accumulated "so THAT's what a day was like" warmth — no dramatic twist required, but every beat must CAUSALLY follow the last (no swapping to an unrelated event). THE SHAPE IS ONE FULL DAY: your cold open is the MOMENT OF WAKING into this day (before dawn — a bird, a bell, first light through the shutters), and the video ends when the same man lies down to sleep that night, content. Make the hook a calm waking moment.`
    ...
    const architectEnding = isScenario
      ? `4. "ending": the END OF THE DAY — the man lying down to sleep that night, CONTENT and at peace, landed on one warm concrete sensory detail (the last of the fire's glow, a full belly, the quiet of the house, a soft blanket). Not a moral, not a summary, not "and that's how it was" — the day simply closes as he drifts off, satisfied. Never end on aching, cold, misery or dread.`
...
3. CAUSAL CHAIN. Every beat follows from the one before — never jump to an unrelated danger or fact (if a beat is about gas, the next isn't suddenly about a flood unless you connect them).`
    ...
    const narratorEnding = isScenario
      ? `\n- SCENARIO ENDING: END THE DAY, CONTENT. The last beat is you lying down to sleep at night, warm and at peace after a good day — do NOT wrap up, zoom out, or reflect. Stop on one warm concrete thing you SEE, HEAR or FEEL as you settle (the last glow of the fire, a full belly, the quiet house, the soft weight of the blanket). The pull to summarize ("and that was life in…", "it makes you wonder…") IS the banned closer. Stop on the image, not a comment on it. Never close on cold, ache, hunger or dread.`
      : ``;
...
- POSITIVE, PEACEFUL LENS — this matters. Describe the day's ordinary chores warmly and contentedly, so a simpler life feels calm, safe and inviting — something the viewer would want to step into. Choose the quiet GOOD in every beat: the warmth of the fire, fresh bread, familiar faces, cool morning air, the satisfaction of honest work, the calm of the fields at dusk. Do NOT dwell on misery, cold, hunger, pain or hardship as the theme, and NEVER editorialise that "life was hard" or "it was so sad". This is NOT hype and NOT over-selling — no "amazing", no forced wonder, no exclaiming over simple things; just a serene, contented, matter-of-fact telling of a good day. The feeling to leave is peace.
