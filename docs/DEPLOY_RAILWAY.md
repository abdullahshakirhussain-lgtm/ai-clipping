...
| `P3009 migrate found failed migrations` | A prior migration attempt was interrupted and left a `failed` row that blocks retries. On an **empty** DB, wipe and let it re-apply: in the Postgres **Query** console run `DROP SCHEMA public CASCADE; CREATE SCHEMA public;` then redeploy. (Lighter alternative, since the migration's DDL is transactional and already rolled back: `DELETE FROM "_prisma_migrations" WHERE migration_name = '0000_init' AND finished_at IS NULL;` then redeploy.) If it fails again at the same statement, read the *migration* deploy log — P3009 hides the real error. |
